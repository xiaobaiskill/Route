...
class Route {
    ...
    public static function check(){
        $depr   =   C('URL_PATHINFO_DEPR');                                                                 // 转移 字符 /
        
        $regx   =   preg_replace('/\.'.__EXT__.'$/i','',trim($_SERVER['PATH_INFO'],$depr));                 //去除后戳.html 
        ...
        // 动态路由处理
        $routes =   C('URL_ROUTE_RULES');                                             //配置的 route 规则                                            
        if(!empty($routes)) {
            foreach ($routes as $rule=>$route){
                if(is_numeric($rule)){                                              //route规则的键
    ...
    private static function checkUrlMatch($regx,$rule) {
        $m1 = explode('/',$regx);             // 原路由 分割成数组
        $m2 = explode('/',$rule);              //规则路由键 分割成数组
        $var = array();         
        foreach ($m2 as $key=>$val){
            if(0 === strpos($val,'[:')){        // 规则路由 循环 查看是否 存在 '[:'      [:id]
                $val    =   substr($val,1,-1);   // 不存在则  第二个到 倒数 第二个       :id
            }
                
            if(':' == substr($val,0,1)) {// 动态变量
                if($pos = strpos($val,'|')){     //查看是否有 或的 形式 :id | action
                    // 使用函数过滤
                    $val   =   substr($val,1,$pos-1);     // 存在 只取 :id
                }
                if(strpos($val,'\\')) {           //是否存在 转义字符 \/  
                    $type = substr($val,-1);      // 存在则 去除 \
                    if('d'==$type) {              //
